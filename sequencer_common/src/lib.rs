...
/// Messages going into the sequencer server and being sent out by the sequencer server over the bus are the same structure, 
...
pub struct SequencerMessage { 
    ...
    pub offset: u64,
    /// On the way into the bus, from senders: This is the per-app sequence number used to deduplicate.
    /// On the bus, sent by the sequencer: Global canonical sequence number for all bus participants
...
impl ArchivedSequencerMessage {
    ...
    pub fn with_sequence_number(self: Pin<&mut Self>, value: u64) -> Pin<&mut Self>{
        // Sequence number is not a reference type and does not contain any reference types,
        // so this unsafe block *should* be good here. 
        unsafe {
